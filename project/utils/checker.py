...
def _piston_urls() -> list[str]:
    raw = os.getenv("PISTON_URLS", "").strip()
    if raw:
        return [item.strip() for item in raw.split(",") if item.strip()]
    custom = os.getenv("PISTON_URL", "").strip()
    if custom:
        return [custom]
...
def _piston_headers() -> dict[str, str]:
    """ส่ง header เฉพาะกรณีตั้งใจใช้ private piston เท่านั้น"""
    force_auth = os.getenv("PISTON_FORCE_AUTH", "0").strip() == "1"
    token = os.getenv("PISTON_API_TOKEN", "").strip()
    if not (force_auth and token):
        return {}
    header_name = os.getenv("PISTON_AUTH_HEADER", "Authorization").strip() or "Authorization"
