...
class RankCog(commands.Cog):
    ...
    def _render_rank(self, category: str) -> discord.Embed:
        data = self.user_store.load().values()

        if category == "python":
            sorted_users = sorted(data, key=lambda x: x.get("languages_cleared", {}).get("python", 0), reverse=True)
            title = "🏆 TOP PLAYERS - Python"
            line_fn = lambda u: f"Python Clear {u.get('languages_cleared', {}).get('python', 0)}"
        elif category == "c":
            sorted_users = sorted(data, key=lambda x: x.get("languages_cleared", {}).get("c", 0), reverse=True)
            title = "🏆 TOP PLAYERS - C Language"
            line_fn = lambda u: f"C Clear {u.get('languages_cleared', {}).get('c', 0)}"
        elif category == "streak":
            sorted_users = sorted(data, key=lambda x: x.get("login", {}).get("consecutive_days", 0), reverse=True)
            title = "🏆 TOP PLAYERS - Streak"
            line_fn = lambda u: f"Streak {u.get('login', {}).get('consecutive_days', 0)} วัน"
        elif category == "coins":
            sorted_users = sorted(data, key=lambda x: x.get("coins", 0), reverse=True)
            title = "🏆 TOP PLAYERS - Coins"
            line_fn = lambda u: f"Coins {u.get('coins', 0):,}"
        elif category == "accuracy":
            sorted_users = sorted(
                data,
                key=lambda x: ((x.get("correct", 0) / max(1, x.get("attempted", 1))) * 100),
                reverse=True,
            )
            title = "🏆 TOP PLAYERS - Accuracy"
            line_fn = lambda u: f"Acc {(u.get('correct', 0) / max(1, u.get('attempted', 1)) * 100):.1f}%"
        else:
            sorted_users = sorted(data, key=lambda x: (x.get("exp", 0), x.get("level", 1)), reverse=True)
            title = "🏆 TOP PLAYERS"
            line_fn = lambda u: f"Lv{u.get('level', 1)} | EXP {u.get('exp', 0)}"

        lines = []
        for index, user in enumerate(list(sorted_users)[:10], start=1):
            lines.append(f"#{index} {user.get('name', 'Unknown'):<12} {line_fn(user)}")
