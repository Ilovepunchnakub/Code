...
from utils.checker import JudgeQueueWorker, JudgeResult, precheck_keywords, judge
from utils.database import AssignmentStore, UserStore
from utils.leveling import apply_rewards, level_progress
from utils.template_engine import build_full_code, match_accepted_code
from views.buttons import ChapterView, LanguageView, QuestCardView, ShopView, SubmissionPreviewView, TaskView
...
class LearnCog(commands.Cog):
    ...
    async def _show_task_card(
        ...
        chapter_number = chapter.get("number", chapter_key.replace("chapter", ""))
        embed.add_field(name="📚 บทเรียน", value=f"บทที่ {chapter_number} {chapter.get('title', '-')}", inline=False)
        embed.add_field(name="🎯 ระดับ", value=str(task.get("difficulty", "ง่าย")), inline=True)
        embed.add_field(name="⭐ รางวัล", value=f"{task.get('xp', task.get('exp', 20))} XP", inline=True)
        embed.add_field(name="💬 คำอธิบาย", value=task.get("description", "-")[:1000], inline=False)

        async def on_submit(sub_interaction: discord.Interaction) -> None:
            await sub_interaction.response.send_modal(
                SubmitCodeModal(
                    on_submit_handler=lambda modal_interaction, code: self._preview_submission(
        ...
        async def on_solution(sub_interaction: discord.Interaction) -> None:
            if not bool(task.get("show_solution", True)):
                await sub_interaction.response.send_message("📘 โจทย์นี้ปิดเฉลย", ephemeral=True)
                return
            await sub_interaction.response.send_message(
                embed=discord.Embed(
                    title="📘 เฉลยหลัก",
                    description=f"```{ 'c' if language == 'c' else 'python'}\n{task.get('main_answer', task.get('example_code', '// ยังไม่มีเฉลย'))}\n```",
        ...
        view = QuestCardView(owner_id=interaction.user.id, on_submit=on_submit, on_hint=on_hint, on_solution=on_solution, on_back=on_back)
        await interaction.response.edit_message(embed=embed, view=view)
    ...
    async def _preview_submission(
        ...
        full_code = build_full_code(language=language, logic_code=logic_code, template_type=str(task.get("template_type", "")))

        async def on_confirm(sub_interaction: discord.Interaction) -> None:
            await self._on_submit_code(sub_interaction, language, chapter_key, task, full_code)

        async def on_edit(sub_interaction: discord.Interaction) -> None:
            await sub_interaction.response.send_modal(
                SubmitCodeModal(
                    on_submit_handler=lambda i, code: self._preview_submission(i, language, chapter_key, task, code),
                    language=language,
                )
            )
        ...
        async def on_cancel(sub_interaction: discord.Interaction) -> None:
            await sub_interaction.response.edit_message(content="ยกเลิกการส่งงานแล้ว", embed=None, view=None)

        embed = discord.Embed(
            title="🧠 ตรวจสอบโค้ดก่อนส่ง",
            description=f"```{ 'c' if language == 'c' else 'python'}\n{full_code[:3500]}\n```",
            color=discord.Color.dark_blue(),
        )
        await interaction.response.send_message(
            embed=embed,
            view=SubmissionPreviewView(
    ...
    async def _on_submit_code(
        ...
        user_data = self.user_store.ensure_user(interaction.user.id, interaction.user.display_name)
        daily_limit = int(task.get("daily_limit", 0) or 0)
        if daily_limit > 0:
            today = date.today().isoformat()
            submit_log = user_data.setdefault("daily_submit_log", {})
            key = f"{language}:{chapter_key}:{task.get('id')}:{today}"
            if int(submit_log.get(key, 0)) >= daily_limit:
                await interaction.response.send_message(
                    embed=discord.Embed(title="⛔ ส่งครบโควตารายวัน", description=f"โจทย์นี้ส่งได้วันละ {daily_limit} ครั้ง", color=discord.Color.red()),
                    ephemeral=True,
                )
                return
            submit_log[key] = int(submit_log.get(key, 0)) + 1

        if not user_data.get("stats", {}).get("first_submission_done"):
            user_data.setdefault("stats", {})["first_submission_done"] = True

        accepted_codes = task.get("accepted_codes") or []
        if accepted_codes and match_accepted_code(code, accepted_codes):
            pass
        ...
        # ใช้ local smart judge เป็นค่าเริ่มต้น เพื่อลดการพึ่ง API ภายนอก
        local_result = judge(user_code=code, lang=language, task=task)
        if local_result.get("status") in {"pass", "fail", "forbidden"}:
            tc = []
            result = JudgeResult(
                verdict="Accepted" if local_result["status"] == "pass" else ("Forbidden" if local_result["status"] == "forbidden" else "Wrong Answer"),
                passed=local_result["status"] == "pass",
                expected_output=str(local_result.get("matched_answer", "-")),
                your_output=code.strip() or "-",
                error_message=str(local_result.get("message", "-")),
                testcase_results=tc,
            )
        else:
        ...
        task_attempts = user_data.setdefault("task_attempts", {})
        task_attempts[task_key] = int(task_attempts.get(task_key, 0)) + 1
        ...
        if result.passed:
            ...
            bonus_xp = int(task.get("bonus_first_try_xp", 0) or 0)
            if first_clear and int(task_attempts.get(task_key, 0)) == 1 and bonus_xp > 0:
                user_data["exp"] = int(user_data.get("exp", 0)) + bonus_xp
                reward["exp"] = int(reward["exp"]) + bonus_xp
            reward_note = (
    ...
    @app_commands.command(name="เรียน", description="เริ่มเรียนเขียนโปรแกรม")
    async def learn_th(self, interaction: discord.Interaction) -> None:
        await self.learn(interaction)
