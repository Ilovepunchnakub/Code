...
from utils.checker import check_code, precheck_keywords
...
class LearnCog(commands.Cog):
    ...
    async def _on_submit_code(
        ...
        result = await check_code(language=language, code=code, task=task)
        ...
        embed.add_field(name="Style Score", value=f"{result.style_score}/100", inline=True)
    ...
    async def daily(self, interaction: discord.Interaction) -> None:
        ...
        embed = discord.Embed(
            title="🎁 Daily Reward",
            description=f"{reward['text']}{jackpot_text}\n🔥 Streak: {user_data.get('login', {}).get('consecutive_days', 1)} วัน",
