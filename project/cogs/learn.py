...
class LearnCog(commands.Cog):
    ...
    @app_commands.command(name="achievement", description="ดูรายการ Achievement ของคุณ")
    async def achievement(self, interaction: discord.Interaction) -> None:
        user_data = self.user_store.ensure_user(interaction.user.id, interaction.user.display_name)
        owned = user_data.get("achievements", [])
        embed = discord.Embed(
            title="🏅 Achievement",
            description="\n".join(f"• {name}" for name in owned[:50]) or "ยังไม่มี Achievement",
            color=discord.Color.gold(),
        )
        embed.set_footer(text=f"ปลดล็อกแล้ว {len(owned)}/100")
        await interaction.response.send_message(embed=embed, ephemeral=True)
