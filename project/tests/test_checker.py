...
def test_check_code_unauthorized_message(monkeypatch) -> None:
    async def fake_execute(**_: object) -> dict[str, object]:
        raise PermissionError("Piston API Unauthorized (401): token invalid")

    monkeypatch.setattr(checker, "_execute_with_retry", fake_execute)
    result = asyncio.run(checker.check_code("python", "print(1)", {"accepted_outputs": ["1"]}))
