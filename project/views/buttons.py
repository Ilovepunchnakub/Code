...
class QuestCardView(OwnerOnlyView):
    def __init__(
        ...
        on_example: Callable[[discord.Interaction], Any],
    ...
    @discord.ui.button(label="💡 ขอ Hint", style=discord.ButtonStyle.secondary)
    async def hint(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        await self._on_hint(interaction)

    @discord.ui.button(label="📖 ตัวอย่าง", style=discord.ButtonStyle.secondary)
    async def example(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        await self._on_example(interaction)
