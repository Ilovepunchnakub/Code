...
class AddTaskPageOneModal(discord.ui.Modal, title="➕ เพิ่มโจทย์ - หน้า 1/3"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        try:
            ...
            await interaction.response.send_message("❌ ค่า EXP ต้องเป็นตัวเลข", ephemeral=True)
            return
...
class AddTaskPageTwoModal(discord.ui.Modal, title="➕ เพิ่มโจทย์ - หน้า 2/3"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        try:
            ...
            await interaction.response.send_message("❌ ค่า Coins ต้องเป็นตัวเลข", ephemeral=True)
            return
        await self.on_submit_handler(interaction, payload)
...
class AddTaskPageThreeModal(discord.ui.Modal, title="➕ เพิ่มโจทย์ - หน้า 3/3"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        try:
            ...
            await interaction.response.send_message("❌ Time Limit / Memory Limit ต้องเป็นตัวเลข", ephemeral=True)
            return
        await self.on_submit_handler(interaction, payload)
...
class EditRewardModal(discord.ui.Modal, title="⭐ แก้ไขรางวัล"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        try:
            payload = {"exp": int(str(self.exp.value)), "coins": int(str(self.coins.value))}
        except ValueError:
            await interaction.response.send_message("❌ ค่า EXP/Coins ต้องเป็นตัวเลข", ephemeral=True)
            return
        await self.on_submit_handler(interaction, payload)
...
class EditJudgeModal(discord.ui.Modal, title="⚙️ แก้ไข Judge"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        try:
            ...
            await interaction.response.send_message("❌ ค่า Judge ต้องเป็นรูปแบบที่ถูกต้อง", ephemeral=True)
            return
        await self.on_submit_handler(interaction, payload)
